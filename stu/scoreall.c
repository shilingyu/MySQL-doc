...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
  ...
  if (sno[0] == '*')
  {
    sprintf(sql, "select sno,cname,score from score_student where sstatus ='1'");
  }
  else
  {
    sprintf(sql, "select sno,cname,score from score_student where sno = '%s'and sstatus='1'", sno);
  }
