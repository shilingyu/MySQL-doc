...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
  ...
  status = cgiFormString("cno",  cno, 32);
	...
	if (sno[0] == '*' && cno[0] == '*')
	{
		sprintf(sql, "select sno,cno,cname from information_couscore where sstatus = '1'");
	}
	else
	{
		sprintf(sql, "select sno,cno,cname from information_couscore where sno = '%s' and cno = '%s' and sstatus ='1'", sno,cno);
	}
